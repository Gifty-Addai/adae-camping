...
const AdminTallowDash: React.FC = () => {
    // isAdmin=true forces logic to use searchProducts (which includes clickCount)
    // We pass initialFilters to ensure we only get Tallow products
    const { products, loading, addProduct, activeProducts, inActiveProducts, totalProducts, editProduct, removeProduct, searchProduct, totalPages, currentPage, goToPage } = useProductAPI(undefined, true, { category: 'tallow' });
    ...
    const handleSaveProduct = async (data: ProductFormData): Promise<boolean> => {
        try {
            let success = false;
            // Force category to be tallow if adding from this page
            if (action === "add" && !data.category) {
                data.category = "tallow";
            }
                    ...
                    <div className="p-4 sm:p-6 lg:p-8 max-w-full mx-auto">
                        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 space-y-4 sm:space-y-0">
                            <div className="flex space-x-2 w-full sm:w-auto">
                                <Input
                                    placeholder="Search tallow products"
                                    value={searchQuery}
                        ...
                        {showModal && (
                            <AdminProductModal
                                ...
                                onDelete={handleDeleteProduct}
                                action={action}
                                defaultCategory="tallow" // Lock category to Tallow
                            />
                        )}
                    </div>
                </div>
...
export default AdminTallowDash;
